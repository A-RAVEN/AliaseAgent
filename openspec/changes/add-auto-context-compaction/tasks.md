...
- [ ] **R-A2-5c 【生产返工，超出 R-A2 测试范围】恢复"累计>阈值→整批一次压"(修复 seam 整体替换批结构的执行偏离)**
  ...
  - **设计决策(改法)**：给 seam 加**双重守卫**(任一失败 → **回退整段确定性算术骨架**)——①**尺寸守卫**：任一段 raw < `max(1, T~/2, 1025)` (`1025`=summary 上限 `max_tokens=1024`+1；真实压缩边界是 1024 而非 T~/2，`T~/2` 在小预算可能 <1024，如 A-2 自调 T~/2≈957)即判"切成会压不动的小段"；②**锚定守卫**：每个 seam 必须落在**对应算术批边界**的邻域窗口内(如 ±该批的一部分)，否则判"**wholesale 重排批结构**"(正是本任务点名要修的 `整体替换批边界` 偏离)→ 拒。两守卫都要，否则尺寸守卫只挡"极小段"，一个段都够大的 seam 仍可整体重排(其构成仍是"一缝一段各自压"——偏语义)。**seam SELECTOR 的候选集须只含"各算术批边界附近"的安全候选**(不从全量安全集任意挑)，prompt 明示"从对应批边界附近挑出 count 个"。约束 seam 只影响"把边界挪到最近话题缝"，不改变批的构成/数量。**L2 分组同样不得被 seam 改成极小 L1 组**。**注意**：双守卫对"批≥~T 且 >1024"的大规模折叠才有意义；小预算/小对话批必 <1025 → seam 一律回退算术(正确——小对话无需话题微调)。**不做**"含钥匙批压不动就原样发送"——D3(design.md:41)/⑪-E 已定论原子批"省略(投影不发、数据留盘)"；"原样发送"会把最旧原文插进投影、破坏 `[摘要][verbatim 尾]` 结构与 recency 梯度(design.md:28)，且需投影层 verbatim-就地-交错重构(design.md:41 "本轮不做")。故含钥匙批的命脉靠**双守卫**，而非改 omit 语义。
...
- [ ] **R-A2-5d 【生产返工，超出 R-A2 测试范围】让工具轮输入(含绝对路径)真正到达摘要器**(修复 content-empty tool-call assistant 被折叠输入丢弃)
...
- [ ] **R-A2-5 回归 + 运行验证**：`flutter test test/integration/`（offline 全绿 + headless live 已删）+ `flutter test --tags live --run-skipped integration_test/compact_quality_live_test.dart -d windows`（窗口 live 通过 + 读到真实摘要含 key）+ `flutter analyze`（我的文件 0 error）。依赖 R-A2-3/4/R-A2-5c。
- [ ] **R-A2-6 诚实审查**：对本返工所有新增/改动开 Workflow 对抗验证（offline，N≥3 REFUTE-skeptics + majority-kill），确认合规 + 无新矛盾 + **未降级**（真实摘要质量断言保留在窗口 live，未下放 offline Fake）。依赖 R-A2-5。
