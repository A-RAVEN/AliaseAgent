...
The system SHALL drive the browser programmatically (via browser-process protocol, not screen-level mouse simulation). Whether commands still execute and the page state stays readable while the window is minimized/occluded is external browser behavior: the tool SHALL confirm this by measurement (change task 1.2) before treating it as guaranteed; if it does not hold, the tool SHALL apply mitigating flags (e.g. `--disable-backgrounding-occluded-windows`) and SHALL NOT represent off-screen execution as guaranteed.
...
- **THEN** if task 1.2 confirmed it, the commands execute in the browser process and the page state is readable programmatically; if task 1.2 recorded the window being re-raised or commands not executing, the tool applies the mitigating flags and the outcome is recorded, not asserted as guaranteed
...
The system SHALL NOT call per-step bring-to-front. The system SHALL implement, via explicit popup/new-tab close handler + deny-downloads/permission-denial configuration, the mechanisms for single-tab reuse, `alert`/`confirm` auto-handling, and download/permission suppression. Whether, on a triggering page, those mechanisms actually hold (no new window/tab, no dialog, no download/permission granted) is external browser behavior: IF (and only if) change task 1.4 confirms they hold via the configured handler, THEN single-tab/no-new-window/no-dialog/no-download/no-permission is guaranteed; otherwise the tool SHALL apply the measured compensating launch args/handlers, record the outcome, and SHALL NOT represent single-tab/no-new-window/no-dialog/no-download/no-permission as guaranteed. The tool SHALL NOT raise the window on its own per-step actions; any deliberate raise requires a separately-defined visibility-request mechanism, which is out of this change's scope (a future change).
...
- **THEN** the tool applies its explicit popup/new-tab close handler + deny-downloads/permission-denial config and auto-dismisses alerts/confirms; whether single-tab reuse / no new window/tab / no dialog / no download / no permission-granted holds is confirmed by task 1.4 (recorded and observable); if task 1.4 does not confirm it, the tool SHALL NOT represent suppression as guaranteed
