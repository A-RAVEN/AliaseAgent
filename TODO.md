...
**已确认的关键性质（这几轮问答定下；其中“最小化/遮挡/抑制”为外部未验证、待 spike 回填）**：
- **AI 走程序级控制**（Playwright/CDP 命令直达浏览器进程，非屏幕级模拟）。**窗口最小化/被遮挡**是否影响 AI 操作、命令是否仍执行、快照是否仍准，是**外部未验证行为**，需实测（对应 change 的 task 1.2）后回填，不作“已确认”。
- **默认藏在后台、不频繁弹窗**：不每步调 `bringToFront`；**单 tab 复用**不开新窗口；**自动处理** `alert`/`confirm` 对话框；避免触发下载 / 权限弹窗——上述“单 tab/不弹窗/无下载/无权限是否真能保持”为**外部未验证**，需实测（task 1.4）回填。把窗口提到前台仅通过显式可见请求，该机制**不在本 change 范围**（另行立项）。
- **健壮性（公共验收标准，须写进实现）**：看门狗**检测关窗→可检测失败→AI 重开会话**；**每步重读实时快照**（不信任上次）；URL / 内容**分叉检测**；**用户接管信号**（检测到用户手动导航/切页 → 知会 AI 重读）。
